...
// Catálogos movidos fuera de execute(): antes se recreaban en cada llamada a
// !cazar/!minar/!pescar/!vender (decenas de objetos literales cada vez), lo cual
// desperdiciaba CPU/RAM en cada ejecución. Ahora se crean 1 sola vez al cargar
// el módulo, igual que ya se hacía en mascotas.js.
const ANIMALES_CAZA = [
...
module.exports = {
    ...
    async execute(sock, chatId, msg, args, { start, cmd, txt, isGroup, sender, db, botState }) {
        ...
        if (start === '!minas') {
            if (botState.juegos[chatId]) return sock.sendMessage(chatId, { text: '⚠️ Juego activo.' });
            const bombs = []; while (bombs.length < 3) { let r = Math.floor(Math.random() * 9); if (!bombs.includes(r)) bombs.push(r); }
        ...
        // !cazar (usa el catálogo ANIMALES_CAZA a nivel de módulo, en vez de
        // recrear el array completo en cada ejecución del comando)
        if (start === '!cazar') {
            const animales = ANIMALES_CAZA;
        ...
        // !minar (usa el catálogo MINERALES_MINA a nivel de módulo)
        if (start === '!minar') {
            const minerales = MINERALES_MINA;
        ...
        // !pescar (usa el catálogo PECES_PESCA a nivel de módulo)
        if (start === '!pescar') {
            const peces = PECES_PESCA;
        ...
        if (start === '!duelo') {
            ...
            if (!ment[0]) return sock.sendMessage(chatId, { text: '⚔️ Menciona a alguien: *!duelo @user [apuesta]*' }, { quoted: msg });
            if (ment[0] === sender) return sock.sendMessage(chatId, { text: '❓ No puedes pelear contigo mismo.' }, { quoted: msg });

            const apuesta = Math.min(Math.abs(parseInt(args[1])) || 500, 1000000);
            ...
            if (b1 < apuesta) return sock.sendMessage(chatId, { text: '💸 No tienes suficientes dikys para apostar.' }, { quoted: msg });
            if (b2 < apuesta) return sock.sendMessage(chatId, { text: '💸 Tu oponente no tiene suficientes dikys.' }, { quoted: msg });

            const n1 = u.nombre_wa || u.nombre || sender.split('@')[0];
            const n2 = target?.nombre_wa || target?.nombre || ment[0].split('@')[0];

            const intros = [
                '🔥 ¡La arena está en llamas!', '⚡ ¡El choque de titanes comienza!',
                '🌩️ ¡Una tormenta de poder se desata!', '🐲 ¡Los dragones de guerra han despertado!'
            ];
            const tecs = [
            ...
            await db.sumarMonedas(ganador, apuesta);
        ...
        // !vender <item> [cantidad] (usa el catálogo VDATA_VENTA a nivel de módulo)
        if (start === '!vender') {
